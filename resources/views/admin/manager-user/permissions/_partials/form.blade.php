...
<div class="bg-white px-4 pb-4 sm:p-6">
    <div class="grid grid-cols-1 sm:grid-cols-3 sm:gap-4 pb-2">
        <div class="col-span-3 sm:col-span-1">
            ...
            class="form-control mt-1 block w-full rounded-md sm:text-sm text-gray-500 border-gray-300 bg-gray-50" disabled="true" value="{{ str_pad($permission->id ?? old('id'), empty($permission) ? 0 : 4 , '0' , STR_PAD_LEFT) }}">
        </div>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-3 sm:gap-4 pb-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 sm:gap-4">
        <div class="col-span-2 sm:col-span-1 pb-2 sm:pb-0">
            ...
            class="form-control mt-1 block w-full rounded-md sm:text-sm text-gray-500 border-gray-300 bg-gray-50" disabled="true" value="@if(!empty($permission)){{$permission->created_at}}@else{{old('created_at')}}@endif">
        </div>
        <div class="col-span-2 sm:col-span-1">
            ...
            class="form-control mt-1 block w-full rounded-md sm:text-sm text-gray-500 border-gray-300 bg-gray-50" disabled="true" value="@if(!empty($permission)){{ $permission->updated_at}}@else{{old('updated_at')}}@endif">
        </div>
    </div>
</div>
