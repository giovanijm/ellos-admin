...
<div class="bg-white dark:bg-slate-900 px-4 pb-4 sm:p-6">
        ...
        @if($customer->photo)
            <div class="grid grid-cols-1 sm:grid-cols-3 sm:gap-4 py-2">
                <div class="col-span-1">
                    ...
                    <a href="{{ route('admin.user.destroyphoto', $customer->id) }}">
                        <x-danger-button
                            type="button"
                            icon="codicon-trash"
                            id="btnRemovePhoto"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 sm:gap-4 py-2">
        ...
        <div class="col-span-1">
            <x-input-label for="documentNumber" class="block mb-1 text-sm font-bold text-gray-700" :value="'* '.__('Número do Documento (CPF)').':'"/>
            <div class="relative w-full">
                <div class="relative w-full">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
    ...
    <div class="grid grid-cols-1 sm:gap-4 py-2">
        <div class="col-span-1">
            ...
            <div class="relative w-full">
                <div class="relative w-full">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <x-fas-book class="w-5 h-5 text-gray-500 dark:text-gray-400"/>
                    </div>
                    <x-text-input class="sm:text-sm w-full pl-10 p-2.5 placeholder-gray-600"
                        id="observation"
                        type="text"
                        name="observation"
                        :value="$customer->observation ?? old('observation')"
                        placeholder="Digite uma observação sobre o cliente..."
                        autofocus
                    />
                </div>
                <x-input-error :messages="$errors->get('observation')" :enableIcon=true/>
            </div>
        </div>
    </div>
